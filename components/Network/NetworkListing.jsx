...
import {NETWORK_CREATE, NETWORK_UPDATE} from "@/utils/routes/routes";
...
const NetworkListing = () => {
  ...
  const [networkUuid, setNetworkUuid] = useState("");
  ...
  const getNetworks = useCallback(
    async (page, q, _perPage) => {
      const response = await NetworkService.getAll(token, page, q, _perPage);
  ...
  useEffect(() => {
    getNetworks(currentPage, qNetWork, perPage);
  }, [getNetworks, qNetWork, currentPage, perPage]);*/
  ...
  const getNetworks = useCallback(
    async () => {
      ...
      const url = new URL(`${BASE_URL}/network?from_listing=0&to_list=1`);
  ...
  const handleDeleteNetwork = async () => {
    ...
    // request delete api
    const {status, message, error} = await networkService.deleteNetwork(token, networkUuid);
    ...
    if (status === 200) {
      ...
      setNetworks(networks.filter(network => network.uid !== networkUuid));
      setInDeleting(prev => !prev)

    } else {
      console.log("delete network response error ||| ", error);
  ...
  //  table data
  let tableData = (networks) => {
    return networks?.map(network => ({
      code: network.code,
      name: network.name,
      parent: network.parent?.name || "",
      inspector:
        network.inspector === null
          ? ""
          : `${network.inspector?.last_name ?? ""} ${network.inspector?.first_name ?? ""}`,
      description: network?.description || "",
      actions: (
        <Stack direction="row" spacing={1}>
          {((!network?.parent && UtilMethods.isAdmin() || network?.parent && (UtilMethods.isDCOM() || UtilMethods.isPDG())) && UtilMethods.getHabilitations(authorizations, 'network').canUpdate) && <Tooltip title={t("update")} placement="bottom">
            <Link href={NETWORK_UPDATE(network.uid)}>
              <EditIcon
                  color="primary"
          ...
          {((!network?.parent && UtilMethods.isAdmin() || network?.parent && (UtilMethods.isDCOM() || UtilMethods.isPDG())) && UtilMethods.getHabilitations(authorizations, 'network').canDelete) &&<Tooltip title={t("delete")} placement="bottom">
            <DeleteIcon
                onClick={() => {
                  setNetworkUuid(network.uid);
                  setDeleteModal(true);
                }}
                color="error"
                className={styles.clickableIcon}
  ...
  const _columns = useMemo(
    () => [
      ...
      },
      {
        ...
        header: NetworkService.roleLabel(t, true),
        size: 150,
        muiFilterTextFieldProps: ({column, rangeFilterIndex, table}) => ({
          inputProps: {placeHolder: t("filter")},
        }),
  ...
  const mrTable = useMaterialReactTable({
    ...
    renderTopToolbarCustomActions: ({table}) => (
      <Box sx={{display: "flex", gap: "1rem", p: "4px"}}>
        {(UtilMethods.getHabilitations(authorizations, 'network').canCreate) && <Button
          variant="contained"
          onClick={() => {
            context.togglePageLoading(true);
            router.push(NETWORK_CREATE);
          }}
          className="__flex_item __right __text-transform-none">{t("createNewBusinessNetwork")}
        </Button>}
      </Box>
    ),
        ...
        <IconButton
          onClick={() => {
            ...
            const tab = selectedRows.reduce((acc, item) => {
              acc.push(networks[item.id]);
              return acc;
            }, []);
            const exportableRows = tab.length > 0 ? tab : networks;
        ...
        <>
          ...
          <Dialog
            ...
            <DialogActions>
              ...
              <Button variant="contained" onClick={handleDeleteNetwork} color="error">
                {t("delete")}
              </Button>
            </DialogActions>
          </Dialog>
