...
import validationSchemaToUpdate from "@/components/Distribution-area/validationSchemaToUpdate";
...
const DistributionAreaUpdate = (callback, deps) => {
  ...
  // get current network
  const getCurrentArea = useCallback(async () => {
    const {area, error} = await DistributionAreaService.getOne(token, getUid());
  ...
  const getNetworks = useCallback(
    async (page, qNetwork) => {
      const {error, networks} = await NetworkService.getAll(token, page, qNetwork);
  ...
  }, [getCurrentArea, getProviders, getNetworks]);
  ...
  }, [getProviders, qProvider, area]);

  useEffect(() => {
    if (area) {
      getNetworks(1, qNetwork);
    }
  }, [getNetworks, qNetwork, area]);
  ...
  const {
    ...
    resolver: zodResolver(validationSchemaToUpdate(t)),
    mode: "all",
  });

  useEffect(() => {
    // Set default values for the form based on the current area
    if (area) {
      const defaultValues = {
        code: area.code,
        name: area.name,
  ...
  useEffect(() => {
    if (area && networks && networks.length === 1) {
      if (!area.reseau) {
        setValue("reseau", networks[0]);
      }
    }
  }, [networks, area, setValue]);

  //  on form submission
  const onSubmit = async data => {
    console.log("update area --- response ||| ", data);
    //  show loading indicator
    setInProgress(true);

    // request api to add provider
    const response = await DistributionAreaService.update(token, getUid(), data);

    if (response.status === 200) {
      context.togglePageLoading(true);

      // show success message
      Toast.success(t("DistributionZoneSuccessfullyUpdated"));

      // redirect to area list page
      router.push(AREA_LIST);
    } else {
      if (response.status === 500) {
        Toast.show(
          t("CannotAssignThisPersonAsManager"),
          5000,
          "error",
        );
      } else {
  ...
  return (
    <>
      {networks === undefined || providers === undefined ? (
        <CreateFormSkeleton />
      ) : (
        <>
          <Button
