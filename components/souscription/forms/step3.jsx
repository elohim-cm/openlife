...
const SousFormStep3 = ({ onContinue, onBack, data = [], filiationsTab = [], isContract = false }) => {
  ...
  useEffect(() => {
    ...
    if (params.length) {
      params.forEach(item => {
        if (item.label === "test") {
          if (SousTestData[item.value]) initForm(SousTestData[item.value].step3);
        }
      });
    }
    setListe(data);
  ...
  const personAlreadyAdded = _data => {
    let element = liste.filter(item => {
      return (item.email === _data.email && _data.email !== "") ||
        (item.phone1 === _data.phone1 && _data.phone1 !== "") ||
        (item.uid === _data.uid && _data.uid !== "");
    });
  ...
  return (
    <div style={{width: "100%"}}>
      <Grid container spacing={2} sx={{mt: 0}}>
        ...
        <Grid xs={12} sm={12} md={12} lg={6} xl={6}>
          <Button fullWidth variant="text" size="large" onClick={() => addItemToList()}>
            <Add />
            &nbsp;{t("add")}
          </Button>
        </Grid>
