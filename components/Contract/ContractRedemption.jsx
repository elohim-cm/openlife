...
const ContractRedemption = ({focus, contract}) => {
  ...
  const fetchRecord = useCallback(async () => {
    ...
    const url = new URL(`${BASE_URL}/contract/${contract.uid}/redemptions`);
    ...
    try {
      ...
      if (response.status === 200) {
        setRecords(response.data.data.redemptions);
  ...
  const mrTable = useMaterialReactTable({
    ...
    renderTopToolbarCustomActions: ({table}) => (
      <Box sx={{display: "flex", gap: "1rem", p: "4px"}}>
        <Typography>{t("contractRedemption")}</Typography>
...
export default ContractRedemption;
