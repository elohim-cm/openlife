...
const DashboardManager = ({dashboard, onRefresh}) => {
  ...
  useEffect(() => {
    if (dashboard != null && dashboard.subscription != null) {
      setPageData({
        ...
        subscription: dashboard.subscription[0].obj_animation_teams
          .filter(item => {
            return item.animation_team_name !== undefined;
          })
          .map(item => ({
            name: item.animation_team_name,
        ...
        collection: dashboard.collection[0].obj_animation_teams
          .filter(item => {
            return item.animation_team_name !== undefined;
          })
          .map(item => ({
  ...
  const columns = [
    {
      accessor: "name",
      Header: () => <span>{t("animationTeam")}</span>,
      isSorted: true,
    },
    {
